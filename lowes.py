# -*- coding: utf-8 -*-
"""
Created on Sat Feb 29 20:12:22 2020
...
db_path="Form99.db"
roo=Tk()
roo.title("login page")
roo.configure(background="navy blue")
roo.geometry("600x600")


def hi():
    if(a.get()=="login" and b.get()=="123"):
        messagebox.showinfo("INFORMATION","login sucessfull")
        #jas()
        roo.destroy()
            
    elif(a.get()!="login"):
        messagebox.showinfo("INFORMATION","username wrong")
    elif(b.get()!="123"):
        messagebox.showinfo("INFORMATION"," password wrong")
    elif(a.get()!="login" and b.get()!="123"):
        messagebox.showinfo("INFORMATION","login failure")

a=StringVar()
b=StringVar()
def thor2():

    tit=Label(roo,text="product search engine",bd=1,relief="solid",font="arial 40",fg="orange",bg="black")
    tit.grid(row=0,column=5,columnspan=2,padx=(40,0),pady=(10,0))
    tit1=Label(roo,text="login",bd=1,relief="solid",font="arial 20")
    tit1.grid(row=1,column=0,padx=(5,0),pady=10)
#abc=frame(root,bg="powder blue",bd=20,relief=RIDGE)
    
    w=Label(roo,text="username",bd=8,padx=2)
    ...
    e1=Entry(roo,show="*",textvariable=b)
    e1.grid(row=4,column=1)

    w1=Label(roo,text="password",bd=8,padx=3)
    w1.grid(row=4,column=0)


    b1=Button(roo,text="login",bd=8,command=hi)
    b1.grid(row=5,column=0)

    b2=Button(roo,text="quit",bd=8,command=roo.destroy)
